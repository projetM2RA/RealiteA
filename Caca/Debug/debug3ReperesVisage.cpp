...
bool detecterVisage(cv::Mat* imCalibColor, Chehra *chehra, std::vector<cv::Point2f> *pointsVisage, std::ofstream *file, std::vector<double> *angles)
{
    ...
    std::cout << "Pitch = " << (*chehra).getPitch() << " ; Roll = " << (*chehra).getRoll() << " ; Yaw = " << (*chehra).getYaw() << std::endl;
    (*file) << (*chehra).getPitch() << " ; " << (*chehra).getRoll() << " ; " << (*chehra).getYaw() << std::endl;

    (*angles).push_back((*chehra).getPitch());
    (*angles).push_back((*chehra).getRoll());
    (*angles).push_back((*chehra).getYaw());
...
//    pointsVisage3D.push_back(cv::Point3f(338, 243, -70)); // exterieur oeil droit sur l'image
    ...
    */
    ...
    std::ofstream file2;
    file2.open ("../rsc/anglesChehra.txt");
    file2 << "   Pitch   " << "   Roll   " << "   Yaw   " << std::endl;
    ...
    do
    {
        ...
        detectionVisage = detecterVisage(frame, &chehra, &pointsVisage2D, &file2, &angles);
...
//            double t1 = tvecs.at<double>(0, 0);
            ...
            double t3 = -tvecs.at<double>(2, 0);
            double t2 = tvecs.at<double>(1, 0) - t3 / correcteurY;
            double t1 = tvecs.at<double>(0, 0); // + t2 / correcteurX;
            ...
            // Valeurs d'angles Chehra
            double roll = angles[1];
            ...
            osg::Matrixd matrixR3;
            matrixR3.makeRotate(roll, osg::Vec3d(0.0, 1.0, 0.0), -yaw, osg::Vec3d(0.0, 0.0, 1.0), -pitch, osg::Vec3d(1.0, 0.0, 0.0));
